...
# Override and return 1 if product can prompt
ProductCanPrompt()
{
  if [ "X$PRODUCT_CAN_PROMPT" = "X" ]
  then
    return 0
  fi
  return $PRODUCT_CAN_PROMPT
}
...
SetDebug()
{
  ...
  AddVMOption "-agentlib:jdwp=transport=dt_socket,server=y,address=0.0.0.0:4000"
}
...
GetDefaultJDK()
{
  ...
  #  4) A java in /usr/java/jdk11*
  #  5) A java in /usr/java11* (AIX)
  ...
  then
    ...
    EchoIfVerbose "Current working directory: $PWD:"
    ...
    then
      ...
      then
        ...
        then
          ...
          # See if we can find /usr/java/jdk11*
          EchoIfVerbose "Checking /usr/java/jdk11"
          jdk=`ls /usr/java 2>/dev/null | grep jdk11 | sort -r | head -1`
          if [[ "X$jdk" != "X" && -f "/usr/java/$jdk/bin/java" ]]
          then
            tmpvar="/usr/java/$jdk/bin/java"
            EchoIfVerbose "Found JDK at /usr/java/jdk11: $tmpvar"
          else
            # See if we can find /usr/java11*
            EchoIfVerbose "Checking /usr/java11"
            jdk=`ls /usr 2>/dev/null | grep java11 | sort -r | head -1`
            if [[ "X$jdk" != "X" && -f "/usr/$jdk/bin/java" ]]
            then
              tmpvar="/usr/$jdk/bin/java"
              EchoIfVerbose "Found JDK at /usr/java11: $tmpvar"
            else
...
CheckJDK()
{
  ...
  then
    ...
    # Exit if this product cannot prompt for input.
    ProductCanPrompt
    CanPrompt=$?
    if [ "$CanPrompt" = "0" ]
    then
      echo "JDK installation not found; retry with --verbose to for more information"
      exit 1
    fi
...
LaunchIDE()
{
    ...
    EchoIfVerbose "Launching IDE: $INVOKED_AS $@"
